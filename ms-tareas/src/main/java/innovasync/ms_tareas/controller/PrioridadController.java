...
public class PrioridadController {
    ...
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Lista de prioridades recuperada exitosamente")
    })
    @GetMapping
    ...
    public ResponseEntity<Prioridad> obtenerPorId(
            @Parameter(description = "ID de la prioridad a buscar", example = "1") 
    ...
    public ResponseEntity<Void> eliminar(
            @Parameter(description = "ID de la prioridad a eliminar", example = "1") 
            @PathVariable Long id) {
