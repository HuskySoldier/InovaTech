...
public class HistorialTareaController {
    ...
    public ResponseEntity<List<HistorialTarea>> obtenerPorTarea(
            @Parameter(description = "ID de la tarea para consultar su historial", example = "10") @PathVariable Long tareaId) {
    ...
    public ResponseEntity<HistorialTarea> registrar(
            @Parameter(description = "ID de la tarea a la que se le agregará el historial", example = "10") @PathVariable Long tareaId,
            @io.swagger.v3.oas.annotations.parameters.RequestBody(description = "Descripción del evento o cambio realizado", content = @io.swagger.v3.oas.annotations.media.Content(mediaType = "text/plain", schema = @io.swagger.v3.oas.annotations.media.Schema(example = "Se actualizó la fecha límite de la tarea"))) 
            @RequestBody String descripcion) {
        // Cambiado a HttpStatus.CREATED (201)
